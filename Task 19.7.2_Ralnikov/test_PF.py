...
def test_get_api_key_wrong_email_and_correct_password(email=invalid_email, password=valid_password):
    status, result = pf.get_api_key(email, password)
    # """неверный email и верный пароль"""
    assert status == 403
    assert 'key' not in result
    print('ok')
...
def test_get_api_key_with_wrong_email_and_wrong_password(email=invalid_email, password=invalid_password):
    status, result = pf.get_api_key(email, password)
     # """Неверный email и неверный пароль"""
    assert status == 403
    assert 'key' not in result
    print('ok')
...
# Test 6
def test_add_pet_valid_data_no_photo(name='Пёсик_без_фото', animal_type='Пёсик', age='2'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Пёсель", "собака", "3", "images/pug2.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")

    # Запрос на удаление первого питомца из списка
...
# Test 10
def test_add_new_pet_photo_big_age(name="maga", animal_type="kot", age="2043"):
        _, auth_key = pf.get_api_key(valid_email, valid_password)
